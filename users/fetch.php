...
if(!($_SESSION['nom']) && !($_SESSION['prenom']) && !($_SESSION['mdp'])){
	...
	$mdp=$_SESSION['mdp'];
    $nom=$_SESSION['nom'];
	$query=mysqli_fetch_array(mysqli_query($con, "SELECT * FROM members WHERE nom='$nom' AND mdp='$mdp' ") );
	$id=$query['id'];
	//echo $id;
...
if(isset($_POST["view"])){
	if($_POST['view'] != ''){
		$update_query="UPDATE notif SET notif_status=1, members_id='$id'  WHERE notif_status=0";
	...
	$query1="SELECT * FROM notif WHERE notif_status=0 and members_id='0'";
